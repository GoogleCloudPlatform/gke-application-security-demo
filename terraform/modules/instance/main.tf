...
data "null_data_source" "grant_admin" {
  inputs = {
    command = <<EOF
kubectl get clusterrolebinding gke-tutorial-admin-${var.execution_id}-binding &> /dev/null ||
kubectl create clusterrolebinding gke-tutorial-admin-${var.execution_id}-binding \
...
EOF
  ...
  provisioner "local-exec" {
    command = "gcloud compute scp --project ${var.project} --zone ${var.zone} --recurse ${path.module}/scripts ${var.hostname}:"
  }
}
